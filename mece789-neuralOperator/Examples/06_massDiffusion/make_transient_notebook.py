"""Run once to produce PIDeepONet_massDiffusion_transient.ipynb"""
import json, uuid
...
cells.append(md([
    ...
    "\n",
    "### Key difference from the steady-state notebook\n",
    "\n",
    "| | Steady state | Transient |\n",
    "|---|---|---|\n",
    "| Trunk input | $(x,y)$ | $(x,y,t)$ |\n",
    "| Mollifier | $u_\\theta\\cdot x(1-x)\\sin(\\pi y)+(1-x)$ | same $\\times\\,(1-e^{-t/\\tau})$ + IC term |\n",
    "| PDE residual | $\\nabla\\cdot(D\\nabla C)=0$ | $\\partial_t C - \\nabla\\cdot(D\\nabla C)=0$ |\n",
    "| $\\partial_t$ | — | autograd through trunk |\n",
    "| IC loss | — | not needed (mollifier handles it) |\n",
    "\n",
    "### PDE loss computation per training step\n",
    "1. Sample $N_t=5$ time points uniformly in $(0,T]$.\n",
    "2. For each $t_k$: evaluate $C$ on the $29\\times29$ spatial mesh.\n",
    "3. $\\partial C/\\partial t$: `torch.autograd.grad` through the trunk net w.r.t. the $t$ input.\n",
    "4. $\\nabla\\cdot(D\\nabla C)$: two FDM passes (same as steady state).\n",
    "5. Residual $\\|\\partial C/\\partial t - \\nabla\\cdot(D\\nabla C)\\|^2$ accumulated and averaged.\n",
]))
...
cells.append(code([
    ...
    "        # x: (N, 2) spatial-only points; a: (batch, nx*ny, 1)\n",
    "        a = a.squeeze(-1)\n",
    ...
    "    # Time-gated lifting function:\n",
    "    #   C = u_nn * x*(1-x)*sin(pi*y) * (1 - exp(-t/tau))  +  (1-x)\n",
    "    # IC (t=0), Dirichlet BCs (x=0,1), and Neumann-consistency (y=0,1)\n",
    "    # are all satisfied exactly by construction.\n",
    ...
    "        '''Residual loss for dC/dt = div(D grad C)'''\n",
    "        n_batch = a_batch.shape[0]\n",
    "        if w_pde == 0.:\n",
    "            return torch.tensor(0.)\n",
    "\n",
    "        # D values at the spatial mesh (time-independent)\n",
    "        a_sp   = self.fun_a(x_mesh, a_batch)                        # (batch, N^2, 1)\n",
    "        a_grid = a_sp.reshape(n_batch, N_mesh, N_mesh, 1)\n",
    "\n",
    "        # sample N_t_pde time points uniformly in (0, T]\n",
    "        t_pts = torch.linspace(T / N_t_pde, T, N_t_pde, dtype=self.dtype)\n",
    "\n",
    "        loss_total = torch.tensor(0., device=self.device)\n",
    "\n",
    "        for t_k in t_pts:\n",
    "            # (x, y, t_k) input — requires_grad=True so autograd flows through t\n",
    ...
    "            u = self.mollifier(u, x_st)                             # (batch, N^2, 1)\n",
    "\n",
    "            # dC/dt via autograd w.r.t. the t-column of x_st\n",
    "            grads = torch.autograd.grad(u.sum(), x_st, create_graph=True)[0]\n",
    "            du_dt = grads[..., 2:3]                                 # (batch, N^2, 1)\n",
    "\n",
    "            # spatial divergence: two FDM passes (each trims 1 layer per side)\n",
    ...
    "\n",
    "            # trim du_dt to the same (N-4)^2 interior after two FDM passes\n",
    ...
    "        # x: (batch, n_t*nx*ny, 3)  u: (batch, n_t*nx*ny, 1)\n",
...
# ── Cell 6: prediction + visualisation ───────────────────────────────────────
